...
> 更新时间：2026-07-10
> 本文描述当前代码的实际行为。
> 当前推荐主线：`full pairwise + single-pass reuse + run-local pair cache`。
...
项目不声称找到了 LLVM 全局最优 pass 顺序。所谓 exact，只在配置的 pass set、轮数、状态上限以及 certified batch-state graph 范围内成立。
...
pairwise 主线在元数据中使用 `exact_scope=pairwise_full_matrix`。只有采用 full、没有 `max_pairs` 截断，并且所有 active pair 都有结果时，`pair_matrix_complete` 才是 true；scope 名称本身不能覆盖这个完整性布尔值。
...
条件全部满足时选 exact，否则选 budgeted，并把原因写入 `auto_reason`。
...
2. 读取 `optimized_pipeline.txt`；
3. 从 root 一次性重新执行完整 pipeline；
4. 生成 `replayed_final.ll`；
5. 与 `final.ll` 走同一等价性阶梯；
6. 写 `pipeline_replay.csv`；
7. 更新 summary。
...
Phasebatch 在每个 LLVM IR 状态上先找出 active passes，用完整 AB/BA 动态测试建立 pair-level independence 证据，再把非 commute 关系构造成 conflict graph，从组件的 maximal independent sets 组合 batch candidates；随后用全排列或 permutation DAG 验证 batch-level correctness，只有 correctness classifier 允许的 batch 才能成为 optimizer state DAG 的边；exact 或 budgeted 搜索在这些安全边上扩展状态。单次优化默认按 IR 指令数选择最终状态；staged 模式可以串联小 pass set，并只在已经安全到达的 top-K 终态之间用运行时间重排。最终 pipeline 必须 replay，随后保留结构化证据并清理 `.ll` 与空目录。
